...
# Make helpers in the same folder importable and reuse dataset loaders
script_dir = os.path.dirname(os.path.abspath(__file__))
if script_dir not in sys.path:
    sys.path.append(script_dir)
from two_stage_knapsack_eval import load_data, resolve_default_dataset_dir
...
def compute_single_correction_obj(realPrice, realWeight, predSol, cap):
    realPrice = realPrice.tolist()
    realWeight = realWeight.tolist()
    m2 = gp.Model()
    m2.setParam('OutputFlag', 0)
...
if __name__ == '__main__':
    for capacity in cap_list:
        ...
        for testmark in range(startmark, endmark):
            # Load instances and predicted solutions using the shared loader
            dataset_dir = "../../data/custom_datasets_700/knapsack"
            instances, pred_sols_matrix, _ = load_data(dataset_dir, capacity)
            # Use the first test_case_num instances for evaluation
            instances = instances[:test_case_num]
            pred_sols_matrix = pred_sols_matrix[:test_case_num]

            # Flatten into the format expected by existing helpers
            # true_weight/value are concatenated per-instance blocks of length item_num
            true_weight = instances[:, :, 0].reshape(-1)
            true_price = instances[:, :, 1].reshape(-1)
            ...
            pred_sol_temp = pred_sols_matrix

            for testNum in range(test_case_num):
                pred_sol = pred_sol_temp[testNum, :]
                ...
                corrrlst = compute_single_correction_obj(cur_true_val, cur_true_weight, pred_sol, capacity)
                corr_obj_list.append(corrrlst)

            print("avgTrainLoss: ", sum(np.array(train_loss_list)) / test_case_num, "avgPReg: ",
                  sum(abs(real_obj - np.array(corr_obj_list))) / test_case_num, "avgTOV: ",
                  sum(real_obj) / test_case_num, "avgEOV: ", sum(corr_obj_list) / test_case_num)
